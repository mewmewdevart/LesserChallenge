	...
	<div class="flex flex-col gap-4 text-center">
		...
		{#if previousCountdown !== undefined && showPreviousCountdown}
			...
			<div class="flex gap-4">
				<div>
					<span class="font-mono text-6xl">
						<span id="timerMinutes" class="timer-minutes"
							>{formatTime(previousCountdown).minutes}</span
						>
					</span>
					min
				</div>
				<div>
					<span class="font-mono text-6xl">
						<span id="timerSeconds" class="timer-seconds"
							>{formatTime(previousCountdown).seconds}</span
						>
					</span>
					sec
				</div>
